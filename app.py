...
def generate_data():
    ...
    stages = ['Lead', 'Prospect', 'Qualified', 'Proposal', 'Negotiation', 'Closed Won', 'Closed Lost']
    stage_weights = [0.25, 0.20, 0.18, 0.15, 0.10, 0.07, 0.05]
    categories = ['Electronics', 'Fashion', 'Home & Living', 'Beauty & Health', 'Sports & Outdoors', 'Books & Media']
    cat_weights = [0.25, 0.22, 0.18, 0.15, 0.12, 0.08]
    channels = ['Organic Search', 'Paid Ads', 'Social Media', 'Email Campaign', 'Referral', 'Direct']
    channel_weights = [0.28, 0.22, 0.20, 0.14, 0.10, 0.06]
    regions = ['North', 'South', 'East', 'West', 'Central']
    segments = ['New Customer', 'Returning Customer', 'VIP Customer']
    seg_weights = [0.45, 0.38, 0.17]
    price_ranges = {'Electronics': (50, 1500), 'Fashion': (20, 400), 'Home & Living': (30, 800),
                    'Beauty & Health': (15, 200), 'Sports & Outdoors': (25, 600), 'Books & Media': (10, 100)}

    leads = []
    for i in range(n):
        lead_date = start_date + timedelta(days=random.randint(0, 364))
        ...
        deal_value = round(random.uniform(lo, hi), 2)
        days_in_stage = random.randint(1, 60)
        close_date = lead_date + timedelta(days=days_in_stage + random.randint(0, 30))
        conv_prob = {'Lead': 0.05, 'Prospect': 0.12, 'Qualified': 0.25, 'Proposal': 0.40,
                     'Negotiation': 0.65, 'Closed Won': 1.0, 'Closed Lost': 0.0}[stage]
        quantity = random.randint(1, 10) if segment == 'VIP Customer' else random.randint(1, 5)
        discount_pct = round(random.uniform(0, 0.25), 2) if segment != 'New Customer' else round(random.uniform(0, 0.10), 2)
        revenue = round(deal_value * quantity * (1 - discount_pct), 2)
        csat = round(random.uniform(3.0, 5.0), 1) if stage == 'Closed Won' else None
        sales_rep = f"Rep_{random.randint(1, 10):02d}"
        leads.append({
            'Lead_ID': f'LD-{i+1:04d}', 'Lead_Date': lead_date, 'Close_Date': close_date,
            'Days_In_Pipeline': days_in_stage, 'Sales_Rep': sales_rep, 'Region': region,
            'Channel': channel, 'Customer_Segment': segment, 'Product_Category': category,
            'Pipeline_Stage': stage, 'Unit_Price': deal_value, 'Quantity': quantity,
            'Discount_Pct': discount_pct, 'Revenue': revenue,
            'Conversion_Probability': conv_prob, 'CSAT_Score': csat,
            'Deal_Won': 1 if stage == 'Closed Won' else 0
        })
    ...
    df['Discount_Band'] = pd.cut(df['Discount_Pct'], bins=[-0.01, 0.05, 0.10, 0.20, 1.0],
        labels=['No Discount (0-5%)', 'Low (5-10%)', 'Medium (10-20%)', 'High (>20%)'])
    df['Revenue_Band'] = pd.cut(df['Revenue'], bins=[0, 200, 500, 1000, 999999],
...
selected_channels = st.sidebar.multiselect("Channel", df['Channel'].unique(), default=list(df['Channel'].unique()))
selected_regions = st.sidebar.multiselect("Region", df['Region'].unique(), default=list(df['Region'].unique()))
selected_segments = st.sidebar.multiselect("Segment", df['Customer_Segment'].unique(), default=list(df['Customer_Segment'].unique()))
...
with tab1:
    ...
    with col1:
        ...
        colors_stage = ['#1F4E79', '#2E75B6', '#4A90D9', '#6BAED6', '#9ECAE1', '#2CA02C', '#D62728']
        stage_counts.plot(kind='bar', ax=ax, color=colors_stage[:len(stage_counts)])
...
with tab2:
    ...
    with col2:
        ...
        ch_s = ch.sort_values('Total', ascending=True)
        ax1.barh(ch_s.index, ch_s['Total'], color='#BDD7EE', label='Total Leads')
        ax1.barh(ch_s.index, ch_s['Won'], color='#2E75B6', label='Deals Won')
        ax2 = ax1.twiny()
        ax2.plot(ch_s['Win_Rate_Pct'], ch_s.index, 'ro-', lw=2, label='Win Rate %')
        ax1.set_xlabel('Count'); ax2.set_xlabel('Win Rate %', color='red')
    ...
    with col2:
        fig, ax = plt.subplots(figsize=(8, 5))
        rg_s = rg.sort_values('Win_Rate_Pct', ascending=False)
        ax.bar(rg_s.index, rg_s['Win_Rate_Pct'], color=['#2E75B6', '#4A90D9', '#6BAED6', '#9ECAE1', '#BDD7EE'])
        ax.set_ylabel('Win Rate (%)'); ax.set_title('Region-wise Win Rate', fontweight='bold')
        for i, v in enumerate(rg_s['Win_Rate_Pct']):
    ...
    with col2:
        ...
        rep_s = rep.sort_values('Win_Rate_Pct', ascending=False)
        avg_wr = df_filtered['Deal_Won'].mean() * 100
        clrs = ['#2E75B6' if v > avg_wr else '#FF6B35' for v in rep_s['Win_Rate_Pct']]
        ax.bar(rep_s.index, rep_s['Win_Rate_Pct'], color=clrs)
        ax.axhline(y=avg_wr, color='red', ls='--', label=f'Avg ({avg_wr:.1f}%)')
        ax.set_ylabel('Win Rate (%)'); ax.set_title('Sales Rep Win Rate (Orange = Below Avg)', fontweight='bold')
        ax.legend()
        for i, v in enumerate(rep_s['Win_Rate_Pct']):
    ...
    if len(won_disc) > 1 and len(lost_disc) > 1:
        ...
        with col2:
            ...
            ax.set_xlabel('Discount %'); ax.set_title('Discount Distribution: Won vs Not Won', fontweight='bold')
...
with tab3:
    ...
    def train_models(_df):
        ...
        df_ml = _df.drop(columns=[c for c in drop_cols if c in _df.columns])
        ...
        for name, model in models.items():
            ...
            results[name] = {
                ...
                'feature_importance': list(model.feature_importances_) if hasattr(model, 'feature_importances_') else None
            }
        return results
    ...
    gb = results['Gradient Boosted']
    if gb['feature_importance'] is not None:
        feat_imp = pd.Series(gb['feature_importance'], index=gb['feature_names']).sort_values(ascending=False)
        fig, ax = plt.subplots(figsize=(10, 6))
        top = feat_imp.head(12)
        ax.barh(top.index[::-1], top.values[::-1], color='#2E75B6')
        ax.set_xlabel('Importance'); ax.set_title('Top 12 Feature Importances', fontweight='bold')
        plt.tight_layout(); st.pyplot(fig); plt.close()
...
with tab4:
    ...
    ax.set_ylabel('Accuracy'); ax.set_title('Training vs Testing Accuracy', fontweight='bold'); ax.legend()
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    w = 0.25
    ax.bar(x - w, [results[n]['precision'] for n in model_names], w, label='Precision', color='#2E75B6')
    ax.bar(x, [results[n]['recall'] for n in model_names], w, label='Recall', color='#FF6B35')
    ax.bar(x + w, [results[n]['f1'] for n in model_names], w, label='F1-Score', color='#2CA02C')
    ax.set_xticks(x); ax.set_xticklabels(model_names)
    ax.set_ylabel('Score'); ax.set_title('Precision, Recall & F1-Score', fontweight='bold')
    ...
    for i, name in enumerate(model_names):
        ...
        axes[i].set_title(name, fontweight='bold'); axes[i].set_xlabel('Predicted'); axes[i].set_ylabel('Actual')
...
with tab5:
    ...
    **Customer Segment-wise:** New Customers actually convert slightly better (7.2%) than Returning Customers (4.9%), though VIP Customers generate the highest average revenue ($1,394). No statistically significant bias was found (p=0.36).
    ...
    - The **top features by importance** were Region, Lead Quarter, Quantity, and Customer Segment.
    """)
